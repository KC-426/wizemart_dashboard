...
{numSelected > 0 ? (
      ...
      >
        ...
        <MenuItem sx={{ color: 'text.secondary' }} onClick={()=>{props.setOpenDeleteConfimModal(true); setIsOpen2(false)}} >
          <ListItemIcon>
            <Iconify icon="eva:trash-2-outline" width={24} height={24} />
          </ListItemIcon>
          <ListItemText primary="Delete Category" primaryTypographyProps={{ variant: 'body2' }} />
        </MenuItem>
...
export default function EnhancedTable() {
  ...
  const [drawerEditMainCategory, setDrawerEditMainCategory] = React.useState(false);
  ...
  useEffect(()=>{
    setLoading(true)
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/get/all/category?page=${currentPage}&search=${search}&main_category=${filterMainCategory}`,{headers: {
  ...
  },[drawerAddCategory,drawerEditCategory,render,currentPage,filterMainCategory])
...
const handleDleteCategories = async(value)=>{
  ...
  await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/delete/category`,{data: value},{headers: {
    'Authorization': `token ${editable_config.FRONTEND_VALIDATOR}`,
  },withCredentials:true})
  .then(async res=>{
    // console.log(res)
    console.log("res?.data?.image_need_to_delete With inner object ==>>",res?.data?.image_need_to_delete[0]?.main_category_image)
 ...
 if(res?.data?.status){
   ...
   for(let i=0;i < res?.data?.image_need_to_delete?.length;i++){
      console.log("res?.data?.image_need_to_delete[i]?.main_category_image?.path=>",res?.data?.image_need_to_delete[i]?.main_category_image)
     await deleteImageFromFirebase(res?.data?.image_need_to_delete[i]?.main_category_image?.path,res?.data?.image_need_to_delete[i]?.main_category_image?.image_name,)
  }
  //====== delete images from firebase =======
  setMessage((prev)=>({...prev,type:"success",message:"Category Deleted Successfully !"}))
...
const goToEditCategoryPage =(cat_id)=>{
  ...
  return (
    <>
    <VideoModal title='Learn how to go through your categories?' video_url="https://www.youtube.com/embed/G3jdGT6-8Ls" isOpen={openVideoModal} handleClose={handleCloseVideoModal} /> 
 ...
 {/* #################### SANCKBAR MESSAGE ######################## */}
    ...
    <div className='category-topbar-box ' >
     ...
     Add Category
         ...
         {/*################ MAIN CATEGORY SIDEBAR BUTTON ################*/}
          ...
          <Button onClick={()=>navigate('/dashboard/categories/add/new')} className='add_Category_page_btn' variant="contained"  startIcon={<Iconify icon="carbon:category" />}> 
          Add & Manage Category
               </Button>
         {/*################ ADD CATEGORY SIDEBAR BUTTON ################*/}
         </div>
           ...
           {/* CONFIRM MODAL */}
           <ConfimModal open={openDeleteConfimModal} title="Delete Categories" onYes={()=>handleDleteCategories(selected)} message="Are you sure you want to delete categories?" handleClose={handleCloseConfimModal}  />
       {/* CONFIRM MODAL */}
   <div className='flex flex-justify-between order-top-bar ' >
   <form onSubmit={handleSearch} className='flex' style={{width:"100%"}} >
   <TextField id="outlined-basic" type="search" fullWidth  value={search} onChange={(e)=>setSearch(e.target.value)}  label="Search In Categories" placeholder='Search Anything...' variant="outlined"  InputProps={{
          startAdornment: (
   ...
   <Button className='search-btn' sx={{mx:2,height:54,px:4,}} type="submit" variant="contained" startIcon={<Iconify icon="ic:twotone-search" />}  >
        Search
      </Button>
   </form>
...
<Paper elevation={3} sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar countCategory={countCategory} setOpenDeleteConfimModal={setOpenDeleteConfimModal} numSelected={selected.length} />
        <TableContainer>
          <Table
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <img className='category-table-image' alt="product" src={row?.main_category_image?.image_url ? `${row?.main_category_image?.image_url}` :noImage } />
                          {/* </a> */}
                      </TableCell>
                      {row.category_name ? 
                      <TableCell onClick={()=>goToEditCategoryPage(row?._id)}  className='clickable-text'  style={{textTransform:"capitalize"}} align="left">{row.main_category_name}</TableCell>
                      
                      :
                      <TableCell     style={{textTransform:"capitalize"}} align="left">{row.main_category_name}</TableCell>

                    }
                      <TableCell style={{textTransform:"capitalize"}} align="left">{row.category_name}</TableCell>
                      <TableCell style={{textTransform:"capitalize"}} align="left"><div className='sub-category-table-flex' >{row.subcategory.map((value,index)=>index <3 &&(
                        <p className='sub-category-style' key={value.name} >{value.name?.slice(0,12)+ ".."}</p>
                      ))}</div></TableCell>
        ...
        {row.category_name &&  <AppRegistrationIcon style={{cursor:'pointer'}} fontSize='small' onClick={()=>goToEditCategoryPage(row?._id)} />}
                 ...
                 {!data.length >0 &&   <TableCell colSpan={6}> <div className='search-not-found' >
