...
{numSelected > 0 ? (
        ...
        (<Typography
          ...
          component="div"
        >
          All Customers {`(${props.usersCount})`} 
        </Typography>
      )
        } 
...
console.log("PAGE NUMBER---",currentPage)
  ...
  useEffect(()=>{
    setLoading(true)
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/user/get/alluser?search=${search}&by_status=${filters?.by_status}&date_from=${stateDate[0]?.startDate}&date_to=${stateDate[0]?.endDate}&page=${currentPage}`,{headers: {
      'Authorization': `token ${editable_config.FRONTEND_VALIDATOR}`,
    },withCredentials:true})
    .then(res=>{
      console.log(res)
      // setData((prev)=>([...prev,...res?.data?.alluser]));
...
const handleDleteProducts = async(value)=>{
  ...
  await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/delete/users`,{data: value},{headers: {
    'Authorization': `token ${editable_config.FRONTEND_VALIDATOR}`,
  },withCredentials:true})
  .then(res=>{
    console.log(res)
...
//############################# HANDLE DELETE CATEGORIES FUNCTION #############################
  ...
  return (
    <>
    <VideoModal title='Learn how to go through your customers ?' video_url="https://www.youtube.com/embed/Ef61gtGwU0I" isOpen={openVideoModal} handleClose={handleCloseVideoModal} /> 
 ...
 {/* #################### SANCKBAR MESSAGE ######################## */}
   ...
   <TextField id="outlined-basic" type="search" fullWidth value={search} onChange={(e)=>setSearch(e.target.value)}  label="Search In Customers" placeholder='Search Anything...' variant="outlined"  InputProps={{
          startAdornment: (
   ...
   <Button className='search-btn' sx={{ml:2,mr:1,height:54,px:4}} variant="contained" type='submit' startIcon={<Iconify icon="ic:twotone-search" />} >
        Search
      </Button>
   </form>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell onClick={()=>navigate(`/dashboard/customers/view/${row._id}`)} style={{textTransform:"capitalize"}} className='clickable-text' align="left">{row.username}</TableCell>
                    ...
                    <AppRegistrationIcon style={{cursor:'pointer'}} fontSize='small' onClick={()=>navigate(`/dashboard/customers/view/${row._id}`)}  />
