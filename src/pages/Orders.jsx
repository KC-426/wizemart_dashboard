...
import { convertDateForOrder ,copyContentFunc,getGapBetweenDates} from '../global/globalFunctions';
...
export default function EnhancedTable() {
  ...
  useEffect(()=>{
    setLoading(true)
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/get/all/orders?page=${currentPage}&search=${search}&by_status=${filters?.by_status}&date_from=${stateDate[0]?.startDate}&date_to=${stateDate[0]?.endDate}`,{headers: {
...
const handleDleteOrders = async(value)=>{
  ...
  await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/delete/order/by/id`,{data: value},{headers: {
    'Authorization': `token ${editable_config.FRONTEND_VALIDATOR}`,
  },withCredentials:true})
  .then(res=>{
 ...
 if(res?.data?.status == true){
  setMessage((prev)=>({...prev,type:"success",message:"Orders Deleted Successfully !"}))
...
// ##################### SNACK BAR FUNCTIONs ##################
  ...
  return (
    <>
     <VideoModal title='Learn how to go through your orders?' video_url="https://www.youtube.com/embed/1YniqOGD9LE" isOpen={openVideoModal} handleClose={handleCloseVideoModal} /> 
 ...
 {/* #################### SANCKBAR MESSAGE ######################## */}
                   {/* CONFIRM MODAL */}
          <ConfimModal open={openDeleteConfimModal} title="Delete Orders" onYes={()=>handleDleteOrders(selected)} message="Are you sure you want to delete orders?" handleClose={handleCloseConfimModal}  />
       {/* CONFIRM MODAL */}
    <Box sx={{ width: '100%' }}>
   ...
   <Button sx={{mx:2,height:54,px:4}} className="order_Search_btn" variant="contained" type='submit' startIcon={<Iconify icon="ic:twotone-search" />} >
        Search
      </Button>
   </form>
...
<Paper elevation={3} sx={{ width: '100%', mb: 2 }}>
                ...
                ?.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                           <p className='app_id_and_copy_box'  ><span onClick={()=>copyContentFunc(row.order_id)} className='copy_app_id_style' style={{color:palette.primary.main}} >
                           <Tooltip arrow title="Copy Order ID" placement="top-start"><IconButton  style={{color:palette.primary.main}} size='small'  ><Iconify className='copy-icon' icon="mingcute:copy-fill" /></IconButton>
                            </Tooltip>
                           </span>
                            <span onClick={()=>goToViewOrder(row._id)}  className='id_styles' >{row.order_id }</span> 
                            
                        {convertDateForOrder(row.createdAt)?.split(',')[0] == currentDate?.split(',')[0] && <p className='new_tag_order_list' >New</p> }
                            </p>
                        {/* {row.order_id} */}
                      </TableCell>
                      ...
                      <p style={{color:palette.primary.main,fontWeight:'600'}} > ₹ {row?.delivery_charges + parseInt(row?.order_total)}</p>
                        </TableCell>
      ...
      >
        ...
        <MenuItem value={30}>Cancel</MenuItem>
                      ...
                      <AppRegistrationIcon style={{cursor:'pointer'}} fontSize='small' onClick={()=>goToViewOrder(row._id)}  />
                        </TableCell>
                    </TableRow>
                  );
                })}
