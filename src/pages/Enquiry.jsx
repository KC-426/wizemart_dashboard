...
console.log("modalenquiryData=>",modalenquiryData);
  ...
  useEffect(()=>{
    setLoading(true)
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/get/all/order/enquiries?page=${currentPage}`,{headers: {
...
const handleDeleteEnquiry = async(value)=>{
  ...
  await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/delete/enquires`,{data: value},{headers: {
    'Authorization': `token ${editable_config.FRONTEND_VALIDATOR}`,
  },withCredentials:true})
  .then(res=>{
    console.log(res)
  ...
  setRender(prev=>!prev)
 if(res?.data?.status){
  setMessage((prev)=>({...prev,type:"success",message:"Enquiry Deleted Successfully !"}))
...
//############################# PREVIEW ENQUIRE MODAL FUNCTION #############################
  ...
  return (
    <>
    <VideoModal title='Learn how to go through your enquiry?' video_url="https://www.youtube.com/embed/FgXYnmnV5zg" isOpen={openVideoModal} handleClose={handleCloseVideoModal} /> 
    ...
    <Button onClick={handleOpenVideoModal} variant="text" className='image-guide-btn-text' startIcon={<Iconify icon="logos:youtube-icon" />}>Learn How it Works</Button>
          ...
          {/*===== Popup Modal ====== */}
 <PopupModal handleClose={handleClosePopupModal}open={openPopupModal} data={<EnquiryPreview data={modalenquiryData} handleClose={handleClosePopupModal} onYes={onYesFunction}  confirmBtnName='Image Compress Now' title='Reduce Image Size!! ' message='Thanks, your plan has been upgraded succesfully!!'  />} />
        {/*===== Popup Modal ====== */}
      ...
      >
        ...
        {/* CONFIRM MODAL */}
        <ConfimModal open={openDeleteConfimModal} title="Delete Enquiries" onYes={()=>handleDeleteEnquiry(selected)} message="Are you sure you want to delete enquiries?" handleClose={handleCloseConfimModal}  />
       {/* CONFIRM MODAL */}
    {/* #################### LOADING SPINNER ######################## */}
                ...
                ?.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center" style={{textTransform:'capitalize'}} >{row?.order_id}</TableCell>
                      <TableCell align="center" style={{textTransform:'capitalize'}} >{row?.username}</TableCell>
                      <TableCell align="center">+91-{row?.phone_number}</TableCell>
                      <TableCell align="center">{convertDate(row?.createdAt)}</TableCell>
                      <TableCell align="center" style={{textTransform:'capitalize'}} >{row?.message?.slice(0,20)}...</TableCell>
                      <TableCell align="center">
                      {/* <EditOutlinedIcon fontSize='small' />  */}
                    <VisibilityIcon style={{cursor:'pointer'}} fontSize='small' onClick={()=>handleOpenPopupModal(row)}   />
